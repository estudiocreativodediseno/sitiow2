				...
				<div class="aside left">
					...
					<?php
						...
						foreach ($menu->modules as $m1) {
								$c1=0;
								echo '
										<li'.($modulesId==$m1['moduleId']?' ':'').'>'. 
											'<a href="'.base_url().'admin.php'.strtolower($m1['urlSection']).'?mod='.md5($m1['moduleId']).'" alt="'.$m1['description'].'" title="'.
							...
							foreach ($m1['modules'] as $m2) {
									...
									echo ($c1++==0?'
										<ul>':'').'
											<li>'. 
												'<a href="'.base_url().'admin.php'.strtolower($m2['urlSection']).'?mod='.md5($m2['moduleId']).'" alt="'.$m2['description'].'" title="'.
								...
								foreach ($m2['modules'] as $m3) {
									...
									echo ($c2++==0?'
										<ul>':'').'
											<li>'. 
												'<a href="'.base_url().'admin.php'.strtolower($m3['urlSection']).'?mod='.md5($m3['moduleId']).'" alt="'.$m3['description'].'" title="'.
									...
									foreach ($m3['modules'] as $m4) {
										...
										echo ($c3++==0?'
											<ul>':'').'
												<li>'. 
													'<a href="'.base_url().'admin.php'.strtolower($m4['urlSection']).'?mod='.md5($m4['moduleId']).'" alt="'.$m4['description'].'" title="'.
										...
										foreach ($m4['modules'] as $m5) {
											echo ($c4++==0?'
												<ul>':'').'
													<li>'. 
														'<a href="'.base_url().'admin.php'.strtolower($m5['urlSection']).'?mod='.md5($m5['moduleId']).'"
