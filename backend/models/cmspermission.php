...
class CmsPermission extends CI_Model {
    ...
    function getAllowedUserModules(){
		...
		foreach ($res->rs_array->result() as $row){
			$modules[$row->modulesId] = array(	'moduleId'		=>		$row->modulesId,
												...
												'urlSection'	=>		strtolower($row->urlSection),
	 ...
	 * @return String
     */
    function getHtmlAllowedUserContents($parentStructure=null,$realParentStructure=null){
		 if(!isset($this->session->userdata['username']))
			return '';
		$userGroupId = $this->session->userdata['userGroupsId'];
		...
		$this->db->query("SELECT CAT_ENTRY_STRUCTURES.entryStructuresId, 	CAT_ENTRY_STRUCTURES.name, 		
							...
							FROM CAT_ENTRY_STRUCTURES
								...
								LEFT JOIN		DET_ENTRY_STRUCTURE
									ON 	(".(!is_null($parentStructure)?	"DET_ENTRY_STRUCTURE.subEntryStructureId":
																		"DET_ENTRY_STRUCTURE.entryStructuresId").
																		" 	= 		CAT_ENTRY_STRUCTURES.entryStructuresId)
							WHERE	
								".(!is_null($parentStructure)?"DET_ENTRY_STRUCTURE.subEntryStructureId IS NOT NULL AND 
														DET_ENTRY_STRUCTURE.entryStructuresId = '".$realParentStructure."' AND":"")."
								".(is_null($parentStructure)?"CAT_ENTRY_STRUCTURES.showInMenu = 1 	AND":"CAT_ENTRY_STRUCTURES.showInSubMenu = 1 AND ")."
								CAT_ENTRY_STRUCTURES.active 	= 	1				
							GROUP BY CAT_ENTRY_STRUCTURES.entryStructuresId
		...
		foreach ($rs_query->result() as $row){
			...
			if($aux != $row->entryStructuresId){
				...
				if($row->subEntryStructureId>'0')
					$result .=	$this->getHtmlAllowedUserContents($row->subEntryStructureId,$row->entryStructuresId);
				$result .= '
														</li>
											';
			}
