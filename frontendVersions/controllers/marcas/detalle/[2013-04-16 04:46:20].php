...
class  extends CI_Controller {			
    ...
    function __construct(){
  	...
  	/**
     * Código generado automáticamente por CMS-FRAMEWORK [2013-04-16 04:45:22]
	 ...
	 */
     function index(){
		...
		';
			...
			$output->url_section = base_url().'index.php/marcas/detalle/';
			
			/* Se carga la vista del Controlador*/
			$this->load->view('/marcas/detalle/', $output);
		}catch(Exception $e){
	 ...
	 */
     function id($valueId){
		try{
			...
			$output->fieldTags = $fieldTags;
			$output->title = 'Detalle de marcas - Detalle de marcas';
			$output->url_section = base_url().'/marcas/detalle/';
			/* Se carga la vista del Controlador*/
			$this->load->view('/marcas/detalle/', $output);
		}catch(Exception $e){
			  /* En caso de error, lo mostramos */
...
/* End of file .php */
/* Location: ./application/controllers/marcas/detalle/.php */
