...
class Home extends CI_Controller {			
    ...
    function __construct(){
  	...
  	/**
     * Código generado automáticamente por CMS-FRAMEWORK [2013-04-26 08:53:41]
	 * Home - Home
	 *
	 * @return void
	 */
     function index(){
		...
		';
					...
					$rs = $this->db->select('CAT_LIBRARIES.urlFile, CAT_LIBRARIES.displayName, CAT_LIBRARY_TYPES.extensionFile, CAT_LIBRARY_TYPES.url_path')->
								from('CAT_LIBRARIES')->
								join('CAT_LIBRARY_TYPES','CAT_LIBRARY_TYPES.libraryTypesId = CAT_LIBRARIES.libraryTypesId','INNER')->
								where('MD5(CAT_LIBRARIES.librariesId)','9bf31c7ff062936a96d3c8bd1f8f2ff3')->get();
					$output->libraries = '';
					foreach($rs->result() as $row):
						if($row->extensionFile == 'css')
							$output->library['9bf31c7ff062936a96d3c8bd1f8f2ff3'] = '<link rel="stylesheet" type="text/css" href="'.substr(base_url(),0,strlen(base_url())-1).$row->url_path.$row->urlFile.'" />
		';
						else if($row->extensionFile == 'js')
							$output->library['9bf31c7ff062936a96d3c8bd1f8f2ff3'] = '<script type="text/javascript" src="'.substr(base_url(),0,strlen(base_url())-1).$row->url_path.$row->urlFile.'"></script>
		';
					...
					$rs = $this->db->select('CAT_LIBRARIES.urlFile, CAT_LIBRARIES.displayName, CAT_LIBRARY_TYPES.extensionFile, CAT_LIBRARY_TYPES.url_path')->
								from('CAT_LIBRARIES')->
								join('CAT_LIBRARY_TYPES','CAT_LIBRARY_TYPES.libraryTypesId = CAT_LIBRARIES.libraryTypesId','INNER')->
								where('MD5(CAT_LIBRARIES.librariesId)','1f0e3dad99908345f7439f8ffabdffc4')->get();
					$output->libraries = '';
					foreach($rs->result() as $row):
						if($row->extensionFile == 'css')
							$output->library['1f0e3dad99908345f7439f8ffabdffc4'] = '<link rel="stylesheet" type="text/css" href="'.substr(base_url(),0,strlen(base_url())-1).$row->url_path.$row->urlFile.'" />
		';
						else if($row->extensionFile == 'js')
							$output->library['1f0e3dad99908345f7439f8ffabdffc4'] = '<script type="text/javascript" src="'.substr(base_url(),0,strlen(base_url())-1).$row->url_path.$row->urlFile.'"></script>
		';
					...
					$rs = $this->db->select('url')->
								from('CAT_SECTIONS')->
								where('MD5(CAT_SECTIONS.sectionsId)','o')->get();
					$row =$rs->row();
					$output->urlSections['o'] = $row->url;
