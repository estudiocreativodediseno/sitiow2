...
class Home extends CI_Controller {			
    ...
    function __construct(){
  	...
  	/**
     * Código generado automáticamente por CMS-FRAMEWORK [2013-04-10 04:05:13]
	 * Home - Home
	 *
	 * @return void
	 */
		...
		';
					...
					/*Código generado automáticamente para la instrucción: startloop*/
						...
						$rs_contents = $this->db->select('CAT_ENTRY_CONTENTS.*')->
										...
										where('MD5(entryStructuresId)','')->get();
						
						$this->db->flush_cache();
						$output->_elements = array();
						$cnt = 0;
						foreach ($rs_contents->result() as $row){
							...
							array_push($output->_elements,$dataRow);
						}
						$perPage 	= 0;
						$output->_actual_page = $actualPage = $this->ACTUAL_PAGE;
						$actualPage--;
						if($perPage>0){
							$output->_total_pages = ($cnt-($cnt%$perPage))/$perPage + ($cnt%$perPage>0?1:0);
							if($actualPage*$perPage >= $cnt)
								$output->_elements = array_slice($output->_elements, ($actualPage-1)*$perPage,$cnt%$perPage);   
							else
								$output->_elements = array_slice($output->_elements, ($actualPage-1)*$perPage,$perPage);   
						}else
							$output->_total_pages = 1;
						
						$output->_total_results = $cnt;
