...
class Home extends CI_Controller {			
    ...
    function __construct(){
  	...
  	/**
     * Código generado automáticamente por CMS-FRAMEWORK [2013-04-16 08:59:17]
	 * Home - Home
	 *
	 * @return void
	 */
        	...
        	//inicia function_index_code
					...
					foreach($rs->result() as $row):
						if($row->extensionFile == 'css')
							$output->libraries .= '				<link rel="stylesheet" type="text/css" href="'.substr(base_url(),0,strlen(base_url())-2).$row->url_path.$row->urlFile.'" />
		';
						else if($row->extensionFile == 'js')
							$output->libraries .= '				<script type="text/javascript" src="'.substr(base_url(),0,strlen(base_url())-2).$row->url_path.$row->urlFile.'"></script>
		';
