...
class Home extends CI_Controller {			
    ...
    function __construct(){
  	...
  	/**
     * Código generado automáticamente por CMS-FRAMEWORK [2013-04-10 04:18:30]
	 * Home - Home
	 *
	 * @return void
	 */
     function index(){
		...
		';
					...
					/*Código generado automáticamente para la instrucción: startloop*/
						...
						$this->db->flush_cache();
						$output->escaparate principal_elements = array();
						$cnt = 0;
						foreach ($rs_contents->result() as $row){
							...
							array_push($output->escaparate principal_elements,$dataRow);
						}
						$perPage 	= 0;
						$output->escaparate principal_actual_page = $actualPage = $this->ACTUAL_PAGE;
						$actualPage--;
						if($perPage>0){
							$output->escaparate principal_total_pages = ($cnt-($cnt%$perPage))/$perPage + ($cnt%$perPage>0?1:0);
							if($actualPage*$perPage >= $cnt)
								$output->escaparate principal_elements = array_slice($output->escaparate principal_elements, ($actualPage-1)*$perPage,$cnt%$perPage);   
							else
								$output->escaparate principal_elements = array_slice($output->escaparate principal_elements, ($actualPage-1)*$perPage,$perPage);   
						}else
							$output->escaparate principal_total_pages = 1;
						
						$output->escaparate principal_total_results = $cnt;
