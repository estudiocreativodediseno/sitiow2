...
class Cruceros extends CI_Controller {			
    ...
    function __construct(){
  	...
  	/**
     * Código generado automáticamente por CMS-FRAMEWORK [2013-03-26 09:42:45]
     ...
     function index(){
		...
		';
					...
					/*Código generado automáticamente para la instrucción: startloop*/
						...
						$perPage 	= 1;
						$output->cruceros_actual_page = $actualPage = $this->ACTUAL_PAGE;
						$actualPage--;
						if($perPage>0){
							$output->cruceros_total_pages = ($cnt-($cnt%$perPage))/$perPage + ($cnt%$perPage>0?1:0);
							if($actualPage*$perPage >= $cnt)
								$output->cruceros_elements = array_slice($output->cruceros_elements, ($actualPage-1)*$perPage,$cnt%$perPage);   
							else
								$output->cruceros_elements = array_slice($output->cruceros_elements, ($actualPage-1)*$perPage,$perPage);   
						}else
							$output->cruceros_total_pages = 1;
						
						$output->cruceros_total_results = $cnt;
  ...
  /*No se reconoce la instrucción: elementId*/
			  /*No se reconoce la instrucción: endloop*/
			  /*No se reconoce la instrucción: totalresults*/
			  /*No se reconoce la instrucción: totalpages*/
			  /*No se reconoce la instrucción: paginator*/
			  /*No se reconoce la instrucción: paginatorshort*/
			
        	//termina function_index_code
	
			$output->fieldTags = $fieldTags;
			$output->title = 'Cruceros - Catalogo de Cruceros';
			$output->url_section = base_url().'index.php/prueba/cruceros';
	 ...
	 */
     function id($valueId){
		try{
			...
			$output->fieldTags = $fieldTags;
			$output->title = 'Cruceros - Catalogo de Cruceros';
			$output->url_section = base_url().'/prueba/cruceros';
			/* Se carga la vista del Controlador*/
			$this->load->view('/prueba/cruceros', $output);
		}catch(Exception $e){
			  /* En caso de error, lo mostramos */
